...
| 보안 그룹 | ALB 는 보안 그룹으로 트래픽을 제어합니다. 콘솔은 **VPC 의 기본 보안 그룹을 미리 선택**해 주고, 필요하면 다른 것을 고르거나 추가할 수 있습니다 |
| 모니터링 | Amazon CloudWatch 지표 연동, 요청 추적 |

보안 그룹을 설정할 때 흔히 놓치는 것: 로드 밸런서의 보안 그룹은 **리스너 포트와 상태
확인 포트 모두**에서 등록된 대상과 통신할 수 있어야 합니다.

🆕 **NLB 는 생성 시점이 갈림길입니다.** 보안 그룹 없이 만들면 **나중에 붙일 수 없습니다.**
보안 그룹을 붙여서 만들었으면 그 뒤에는 바꿀 수 있지만 **항상 최소 1개를 유지**해야 해서
전부 제거할 수는 없습니다.
...
| **미사용**(unused) | 대상 그룹에 등록되지 않았거나, 대상 그룹이 리스너 규칙에 쓰이지 않거나, 활성화되지 않은 AZ 에 있거나, 대상이 **중지·종료 상태**임 |
| **사용 불가**(unavailable) | 대상 그룹의 상태 확인이 **비활성화**됨 |

> — 출처: [Health checks for your target groups](https://docs.aws.amazon.com/elasticloadbalancing/latest/application/target-group-health-checks.html)

**그래서 각 대상 그룹의 각 AZ 에 최소 하나의 정상 대상**을 두는 것이 중요합니다. 어떤 AZ 에
정상 대상이 없으면 그 AZ 노드의 IP 가 DNS 응답에서 빠지는데, 이때 무슨 일이 일어나는지는
[7.4 한 AZ 에 장애가 나면 무엇이 일어나는가](#74-한-az-에-장애가-나면-무엇이-일어나는가)
에서 자세히 봅니다.
...
Elastic Load Balancing 의 **현행 세대는 세 가지**입니다. 이 과정은 ALB 와 NLB 를 중심으로
다룹니다.
...
🔄 교재 강사 노트는 "4가지 유형"이라고 세는데, 슬라이드 본문은 위 세 가지만 표시합니다.
현행 문서는 이 셋을 **현행 세대(current generation)** 로 묶고, 네 번째로 세는 것은 **이전
세대** 유형이어서 **현행 세대로 마이그레이션할 것을 권장**합니다. 그래서 이 자료도 세
가지만 다룹니다.

> — 출처: [What is Elastic Load Balancing?](https://docs.aws.amazon.com/elasticloadbalancing/latest/userguide/what-is-load-balancing.html)
...
| **영역별 격리** | AZ 마다 노드와 **별도의 IP·DNS 레코드**를 두어 한 AZ 의 문제가 다른 AZ 로 번지지 않습니다. 영역 격리가 필요하다면 **단일 AZ 구성도 가능**합니다. 고가용성은 **여러 AZ 에 로드 밸런서와 대상을 모두 구성**해야 얻습니다 (7.4 절) |
| **소스 IP 주소 유지** | 백엔드가 클라이언트 IP 를 그대로 봅니다. 다만 **대상 유형이 IP 주소이고 프로토콜이 TCP·TLS 인 대상 그룹은 기본이 꺼짐**입니다 (4.2 절) |
...
| **리디렉션**(redirect) | 한 URL 에서 다른 URL 로 보냅니다. 임시(**HTTP 302**) 또는 영구(**HTTP 301**). URI 구성 요소는 `protocol://hostname:port/path?query` 이고, **`protocol`·`hostname`·`port`·`path` 중 최소 하나를 바꿔야** 리디렉션 루프에 빠지지 않습니다 |
...
**Amazon Application Recovery Controller(ARC)** 의 기능으로, **한 번의 작업으로** 손상된
가용 영역을 서비스 경로에서 빼냅니다.
...
- 시작 전에 **남은 AZ 의 용량을 확인**해야 합니다. 교차 영역을 끈 상태라면 그 AZ 의 대상
  용량이 함께 빠지기 때문입니다

#### 무엇이 빠지는가

로드 밸런서 자체가 옮겨지거나 지워지는 것이 아닙니다. **그 AZ 에 있는 두 가지**가 경로에서
빠집니다.

| 빠지는 것 | 무슨 일이 일어나는가 |
|---|---|
| 손상된 AZ 의 **로드 밸런서 노드** | 그 AZ 의 **노드 IP 가 DNS 에서 제거**됩니다. NLB 문서는 ARC 가 그 로드 밸런서에 **상태 확인 실패를 만들어** 이를 일으킨다고 서술합니다. 7.4 절과 **같은 지레를 운영자가 당기는 것**입니다 |
| 손상된 AZ 의 **대상** | 그 AZ 대상으로 가는 트래픽이 **차단**됩니다. NLB 는 대상을 **비활성화**해 트래픽을 받지 않게 하고 ELB 가 이를 `disabled targets for zonal shift` 로 취급합니다. **상태 확인은 계속 받습니다** |

그래서 영역 이동은 **되돌릴 수 있는 조치**입니다. 대상이 정상이고 이동이 만료되거나 취소되면
그 AZ 로 라우팅이 재개됩니다.

#### 이미 열려 있던 연결은 바로 끊기지 않습니다

DNS 에서 IP 를 빼는 것은 **새 연결**에만 즉시 듣습니다. 손상된 AZ 를 실제로 비우는 데
걸리는 시간은 유형마다 다릅니다.

- **NLB** — 손상된 AZ 의 대상으로 **이미 열려 있던 연결은 자연히 닫힐 때까지 유지**됩니다.
  새 연결만 그 AZ 로 가지 않습니다
- **ALB** — `HTTP 클라이언트 keepalive 기간`이 연결이 얼마나 이어질지를 정합니다.
  기본값이 **3600초(1시간)** 라서 그대로 두면 클라이언트가 손상된 AZ 에 오래 남습니다

AWS 는 ALB 의 keepalive 기간을 **애플리케이션의 복구 시간 목표에 맞춰 낮추라**고 권고합니다.
예시로 **300초**를 듭니다. 값을 고를 때는 **재연결이 잦아져 지연 시간에 영향을 주는 쪽**과
**클라이언트를 손상된 AZ 에서 더 빨리 빼내는 쪽** 사이의 절충으로 보라고 안내합니다.

#### 켜 두어야 씁니다

🆕 두 유형 모두 **기본적으로 꺼져 있고 로드 밸런서마다 켜야 합니다.** 로드 밸런서 속성에서
`ARC 영역 이동 통합`을 활성화합니다.

```bash
# 로드 밸런서에 ARC 영역 이동 통합을 켭니다 (ALB·NLB 공통)
...
| 단일 AZ 대상 그룹 | 영역 이동이 **동작하지 않습니다.** ALB 문서에 명시된 제약입니다 |
...
| 슬라이드 10 본문과 강사 노트 | 본문은 로드 밸런서 유형을 **세 개**(ALB·NLB·GWLB) 표시하는데, 강사 노트는 **"4가지 유형"** 이라고 셉니다 | 현행 문서는 이 셋을 **현행 세대**로 묶고 네 번째는 **이전 세대**로 분류해 현행 세대로 마이그레이션할 것을 권장합니다. 4장에는 슬라이드 본문과 같은 세 유형만 넣었습니다 |
...
| 항목 | 무엇인가 | 근거 |
|---|---|---|
| **영역 이동 (zonal shift)** | 손상된 AZ 의 **로드 밸런서 노드 IP 를 DNS 에서 빼고 그 AZ 대상으로 가는 트래픽을 차단**합니다. ARC 의 기능 | [How Elastic Load Balancing works](https://docs.aws.amazon.com/elasticloadbalancing/latest/userguide/how-elastic-load-balancing-works.html) |
...
| 내부 로드 밸런서의 IP 주소 유형 | "내부 로드 밸런서에는 IPv4 를 사용하는 것이 좋습니다" | **현행 ALB·NLB 문서에서 이 권고를 찾지 못했습니다.** 오히려 IP 주소 유형 문서는 HTTP·HTTPS·TCP·TLS·UDP 대상 그룹 프로토콜이 IPv4 전용과 IPv6 전용을 모두 지원한다고 표로 정리합니다. 교재 권고를 본문에 옮기지 않았습니다 |
