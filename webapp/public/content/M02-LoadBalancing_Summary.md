...
| 값 | 동작 |
|---|---|
| `append` (**기본값**) | 클라이언트 IP 를 `X-Forwarded-For` 에 덧붙여 대상에 전달합니다 |
| `preserve` | 헤더를 받은 그대로 유지합니다 |
| `remove` | 대상에 보내기 전에 헤더를 제거합니다 |
...
| **영역별 격리** | 단일 AZ 아키텍처를 지원하도록 설계되었습니다. AZ 장애 시 AWS 가 자동으로 다른 정상 AZ 로 장애 조치합니다. 다만 고가용성을 위해서는 **여러 AZ 에 로드 밸런서와 대상을 구성하는 것이 권장**됩니다 |
...
이 모듈에서 수치나 동작이 틀린 항목은 없었습니다. 다만 **교재 안에서 서로 어긋나는 서술**이
두 군데 있습니다.

교재 **슬라이드 19** 는 ALB 의 "소스 IP 주소 유지"를 표에서 "예"로 표시하고 각주를
달았지만, 같은 슬라이드 강사 노트는 "ALB 는 소스 IP 주소를 보존하지 않는다"고
서술합니다. 두 서술이 충돌하므로 4.2 절에서 `X-Forwarded-For` 헤더로 설명했습니다.
슬라이드 18 강사 노트의 `TLD 종료`는 `TLS 종료`의 오기로 보입니다.
